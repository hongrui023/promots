...
用一份真实结构的技术手册片段作为夹具（含章节标题、`【3.1】` 编号、分隔线、
`技能：xxx` 元数据行、速查表、说明性章节），检验分块、字段抽取、噪音过滤、
占位符提取、去重三档、幂等性、代码围栏保护与诊断报告。当前 **76 项全部通过**。
...
响应式布局（桌面 + 移动截图）、批量导入全流程（含弹窗布局高度断言）、零控制台错误。
当前 **60 项全部通过**。

> 测试会先注销 Service Worker、清空缓存与 IndexedDB。两步都不能省：
> 不清外壳缓存，PWA 会拿旧代码，测出假结果；不清本地数据，
> 上一轮导入的指令会让这一轮判成「重复」而不再勾选，断言成片失败。
> 清理走 CDP 的 `Storage.clearDataForOrigin`，不在页面里调 `indexedDB.deleteDatabase`
> —— 页面自己持有连接时会触发 `onblocked`，库其实删不掉却返回成功。
...
```bash
# 4) AI 拆分路径 —— 需要模型 API Key，会真实调用并消耗 token
GLM_KEY=<你的key> node tools/test-ai-import.mjs
```

覆盖：连通性、用真实文档夹具做拆分、说明性内容剔除、占位符保留、超长文本分批、
进度回调、无效 Key 的错误处理。当前 **20 项全部通过**，一次约消耗 2 万 token。

> 这条**不进常规自检**。它也顺带验证了一件事：智谱 GLM 的接口返回完整的 CORS 头，
> 所以网页端可以直接调用（微云和百度网盘的接口都拿不到跨域头，只有桌面端能用）。
> Key 只从环境变量读，绝不写入任何文件。
