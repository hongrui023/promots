...
│   ├── gen-pinyin.js         # 生成拼音表
│   ├── serve.js              # 零依赖本地服务器
│   ├── verify.mjs            # 引擎自检（89 项）
│   ├── smoke-browser.mjs     # 浏览器冒烟测试（40 项，CDP 驱动）
...
```bash
node tools/verify.mjs          # 核心引擎：语法 + 89 项功能断言
node tools/smoke-browser.mjs   # 界面：真实浏览器 40 项断言（需先起服务与调试端口）
node tools/weiyun-e2e.mjs --token <你的微云Token> --big   # 微云：真实上传下载 18 项断言
```

当前状态：**147 项断言全部通过**，其中

- 引擎 89 项：拼音、近义词、哈希（SHA1/MD5 对照 Node crypto 逐字节校验）、微云上传参数、
  13 个分类用例（准确率 13/13）、四种搜索模式、过滤器、排序、边界、同步适配器
- 界面 40 项：渲染、四种搜索的交互、侧边栏筛选、编辑器自动分类联动、设置页六个通道与
  微云表单、响应式布局、零控制台错误
